...
En este capítulo se abordan las metodologías de procesamiento digital de señales aplicables a la caracterización de ondas de impulso, en conformidad con las normas IEC 61083-2 \cite{iec61083_2} e IEC 60060-1 \cite{iec60060_1}. Comienza con el análisis de las técnicas de filtrado digital IIR y FIR, e introduciendo la técnica de filtrado de fase cero para mitigar la distorsión de la morfología del impulso. A continuación, se exponen las técnicas de regresión para el ajuste de curvas, enfocándose en la modelación no lineal del algoritmo de Levenberg-Marquardt. Finalmente, se examinan los algoritmos de interpolación, orientados a la determinación precisa de valores continuos a partir de los datos discretos registrados.
...
\begin{itemize}[label={}, leftmargin=*]
    \item $u_d(t)$: función de tensión.
    \item $t$: tiempo.

    Donde los coeficientes desconocidos a encontrar mediante el ajuste son:
...
Donde:
\begin{itemize}[label={}, leftmargin=*]
    \item $w_{i+1}$: vector de parámetros actualizado.
    \item $w_i$: vector de parámetros actual.
    \item $\lambda$: factor de amortiguamiento.
    \item $e$: vector de residuos o errores.
    \item $J$: matriz Jacobiana, formada por las primeras derivadas parciales de los errores respecto a los parámetros del modelo.
    \item $J^T J$: matriz de Hessiana aproximada, compuesta por las segundas derivadas, que modelan la curvatura general de la función de error.
    \item $diag(J^T J)$: matriz diagonal. Escala cada parámetro según la curvatura local de la función.
    \item $J^T e$: vector gradiente de la función error. Indica la dirección de máxima pendiente.
\end{itemize}

\begin{itemize}[leftmargin=*]
    \item Si $\boldsymbol{\lambda \rightarrow 0}$, el algoritmo se comporta como el \textbf{Método de Gauss-Newton}, donde asume que la función de error es localmente cuadrática, y utiliza expansiones de series de Taylor para aproximarse al mínimo de forma muy rápida, pero puede ser inestable y fallar si los valores iniciales elegidos están muy lejos del óptimo. Resultando:
    \begin{equation}
        w_{i+1} = w_i - \left( J^T J \right)^{-1} (J^T e)
    \end{equation}
    \item Si $\boldsymbol{\lambda \rightarrow \infty}$, se comporta como el \textbf{Descenso del gradiente}, que  actualiza los coeficientes moviéndose en la dirección donde el error desciende más rápido. Es muy robusto, es difícil que diverja, pero su convergencia puede ser muy lenta.
    \begin{equation}
