...
Dado que los datos adquiridos por el osciloscopio, possen ruido electromagnético  y errores de cuantización, es necesario aplicar técnicas de ajuste de curvas e interpolación para extraer los parámetros característicos del impulso tipo rayo.

\subsection{Ajuste de curvas}

El ajuste de curvas es un proceso matemático que busca encontrar la función matemática que mejor se aproxima a un conjunto de datos experimentales. En el caso de los impulsos tipo rayo, la norma IEC 60060-1 establece que la forma de la onda debe ajustarse a una función de doble exponencial, lo que permite caracterizar adecuadamente el comportamiento del impulso en términos de sus parámetros fundamentales (amplitud, tiempos de frente y cola y sobrepasamiento.).\par



Para la evaluación del impulso tipo rayo, la norma IEC 60060-1 exige luego de capturar la señal digitalizada, se haga una selección de los datos muestreados en el intervalo de tiempo copmrendido entre el \SI{20}{\percent} del valor pico en el frente y el \SI{40}{\percent} del valor pico en la cola. A este intervalo se le debe ajustar la función de doble exponencial de la \autoref{Eq:DobleExponencial}
\begin{equation}
    u_d(t) = U \cdot \left( e^{-\frac{t-t_d}{\tau_1}} - e^{-\frac{t-t_d}{\tau_2}} \right)
    \label{Eq:DobleExponencial}
\end{equation}
Donde $t$ es el tiempo, $u_d(t)$ es la función de tensión. $U$, $\tau_1$, $\tau_2$ y $t_d$ son los parámetros a encontrar mediante el ajuste.
