...
El presente capítulo establece el marco normativo y conceptual necesario para el desarrollo del software de adquisición y análisis de impulsos de alta tensión. En primera instancia, se definen los conceptos básicos, y los parámetros de la onda de impulso atmosférico tipo rayo \qty[parse-numbers=false]{1,2/50}{\micro\second}. Posteriormente, se detallan los procedimientos algorítmicos para el cálculo de dichos parámetros temporales y de amplitud. Finalmente, se exponen los requisitos técnicos exigidos por la norma IEC 61083-2 \cite{iec61083_2} para la validación de software de evaluación y los principios fundamentales para la estimación de la incertidumbre en sistemas de medición digital.

\section{Definiciones del ensayo de impulso de tensión tipo rayo}
...
\textbf{Función de tensión de ensayo:} Es una función de amplitud-frecuencia que se define para representar la respuesta del aislamiento a impulsos con sobreelevación (overshoot), dada por la \autoref{eq:func_tension_ensayo}, donde $f$ es la frecuencia en \qty{}{\mega\hertz}.
...
\textbf{Tiempo hasta el semi-valor $T_2$:} Es un parámetro virtual definido como el intervalo de tiempo entre el origen virtual ($O_1$), y el instante en que la curva de tensión de ensayo ha disminuido a la mitad del valor de cresta de la tensión de ensayo ($U_\text{t}$).
...
De acuerdo con la norma IEC 60060-1 \cite{iec60060_1}, la tensión de impulso tipo rayo estándar u \enquote{Onda plena} es una tensión de impulso tipo rayo completa y suave, que tiene un tiempo de frente ($T_1$) de \qty{1,2}{\micro\second}, y un tiempo hasta el semi-valor ($T_2$) de \qty{50}{\micro\second}, y se describe como un impulso \qty[parse-numbers=false]{1,2/50}{\micro\second}.

Adicionalmente, la norma establece las tolerancias que acepta en los parámetros medidos, indicados en la tabla \ref{tab:tolerancias_impulso}:
...
El ensayo de impulso, a nivel general, se realiza aplicando una cierta cantidad de impulsos al objeto bajo ensayo, y registrando la respuesta del mismo, mediante un sistema de adquisición de datos, compuesto por un divisor de tensión (resistivo o capacitivo), un atenuador coaxial y un oscilocopio digital.

El análisis de los parámetros del impulso registrado permite evaluar la capacidad del equipo para soportar sobretensiones transitorias referidas a descargas atmosféricas, y determinar su capacidad de aislamiento.

La norma IEC 60060-1, en su Anexo B, describe un método para el cálculo de los parámetros de las ondas de impulso tipo rayo estándar (plenas y cortadas) con o sin sobreelevación u oscilaciones superpuestas. Este método se basa en la estimación de una curva base, que representa la forma idealizada de un impulso tipo rayo sin oscilaciones, y el cálculo de una curva residual, que representa las oscilaciones superpuestas. Posteriormente, se aplica un filtro definido por la función de tensión de ensayo (ecuación \ref{eq:func_tension_ensayo}) para obtener la curva de tensión de ensayo, de la cual se calculan los parámetros del impulso.
